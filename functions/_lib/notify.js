/**
 * RiGeBa Lighting AI — Notification layer (zero dependencies)
 *
...
export async function sendEmailVerbose(env, subject, text) {
  ...
  const from = env.NOTIFY_FROM || 'RiGeBa Lighting AI <onboarding@resend.dev>';
  try {
...
const REPLY_FOOTER = [
  '',
  '---',
  'RiGeBa Lighting — professional stage lighting manufacturer',
...
const FALLBACK_BODY = [
  'Thanks for getting in touch with RiGeBa Lighting.',
  '',
...
export async function sendAutoReplyVerbose(env, lead) {
  ...
  const subject = parsed.subject ||
    'Your stage lighting enquiry — RiGeBa Lighting';
  const body = (parsed.body || FALLBACK_BODY) + '\n' + REPLY_FOOTER;

  const from = env.AUTOREPLY_FROM || env.NOTIFY_FROM || 'RiGeBa Lighting <onboarding@resend.dev>';
  const replyTo = env.REPLY_TO || 'sales20@rigelighting.com';
...
export async function createGithubIssue(env, lead, base) {
  ...
  const body =
    ...
    '### Drafted reply\n\n```\n' + (lead.ai_reply || '_n/a_') + '\n```\n\n' +
    '> Auto-generated by RiGeBa Lighting AI reception agent.';
